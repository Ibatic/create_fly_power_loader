...
Adds mechanical chunk loaders for Create: an Andesite tier (single chunk, static or contraption-mounted) and a Brass tier (configurable radius via scroll wheel, works while attached to train stations). Both loaders are powered by capturing a Ghast inside them.
