...
public class EmptyChunkLoaderBlockItem extends BlockItem {
    ...
    public final Block reward;

    private EmptyChunkLoaderBlockItem(Block block, Properties properties, Block reward) {
    ...
    protected void giveChunkLoaderTo(Player player, ItemStack heldItem, InteractionHand hand) {
        ItemStack filled = new ItemStack(reward);
    ...
    private void spawnCaptureEffects(Level world, Vec3 vec) {
        if (world.isClientSide()) {
            for (int i = 0; i < 80; i++) {
                Vec3 position = VecHelper.offsetRandomly(vec, world.getRandom(), 1.5f);
